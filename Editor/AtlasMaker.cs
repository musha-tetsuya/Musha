...
public class AtlasMaker : EditorWindow
{
    ...
    private List<AtlasParts> m_atlasPartsList = new List<AtlasParts>();
    ...
    private void OnGUI()
    {
        ...
        {
            EditorGUI.BeginDisabledGroup(m_atlasPartsList.Count == 0 || !m_atlasPartsList.Exists(x => x.IsValid()));
            {
                //保存ボタン
            ...
            //Padding
            m_padding = EditorGUILayout.IntSlider(m_padding, 0, 10, GUILayout.Width(169));
        }
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        {
            EditorGUI.BeginDisabledGroup(m_atlasPartsList.Count == 0);
            {
                //クリアボタン
                if (GUILayout.Button("Clear", GUILayout.Width(60)))
                {
                    for (int i = 0; i < m_atlasPartsList.Count; i++)
                    {
                        m_atlasPartsList[i].Delete();
                    }
                    m_atlasPartsList.Clear();
                }
            }
            EditorGUI.EndDisabledGroup();
            ...
            if (tex != null)
            {
                if (m_atlasPartsList.Exists(x => x.Equals(tex)))
                {
                    Debug.LogWarningFormat("Texture2D'{0}' is already exist.", tex.name);
                }
                else
                {
                    m_atlasPartsList.Add(new AtlasParts(tex));
                }
            }
        }
        GUILayout.EndHorizontal();
        ...
        if (m_atlasPartsList.Count > 0)
        {
            //全アトラスパーツ表示
            for (int i = 0; i < m_atlasPartsList.Count; i++)
            {
                if (!m_atlasPartsList[i].OnGUI())
                {
                    m_atlasPartsList[i].Delete();
                    m_atlasPartsList.RemoveAt(i);
                    break;
                }
            }
        }
    ...
    private void Save(string path)
    {
        ...
        m_atlasPartsList.Clear();
        m_atlasPartsList.Add(new AtlasParts(AssetDatabase.LoadAssetAtPath<Texture2D>(path)));
    ...
    private void PackTextures(out Texture2D atlasTex, out SpriteMetaData[] spriteSheet)
    {
        ...
        {
            //パッキングするので読み書き可能に変更
            m_atlasPartsList[i].SetReadable(true);

            //パッキングされるテクスチャたちを取得
            enabledTexList.AddRange(m_atlasPartsList[i].GetEnabledTexture());
        }

        //テクスチャをパッキング
        var packedTex = new Texture2D(1, 1);
        var packedRects = packedTex.PackTextures(enabledTexList.ToArray(), m_padding, 2048);

        //パッキング終わったので読み書き設定を元に戻す
        for (int i = 0; i < m_atlasPartsList.Count; i++)
        {
            m_atlasPartsList[i].RevertReadable();
        }
